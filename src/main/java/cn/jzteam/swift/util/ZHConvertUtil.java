...
public class ZHConvertUtil {
    ...
    private static final ZHConvertUtil[] converters = new ZHConvertUtil[2];
    private static final String[] propertyFiles = new String[2];

    public static ZHConvertUtil getInstance(int converterType) {
        if (converterType >= 0 && converterType < 2) {
            if (converters[converterType] == null) {
                Class var1 = ZHConvertUtil.class;
                synchronized(ZHConvertUtil.class) {
                    if (converters[converterType] == null) {
                        converters[converterType] = new ZHConvertUtil(propertyFiles[converterType]);
                    }
                }
            }
    ...
    public static String convert(String text, int converterType) {
        ZHConvertUtil instance = getInstance(converterType);
        return instance.convert(text);
    }

    private ZHConvertUtil(String propertyFile) {
        InputStream is = null;
